...
OTU_table <- read.csv("OTU_table_AWI2.csv", sep=";", header = TRUE, stringsAsFactors = FALSE, check.names = FALSE)

OTU_table$Unlabeled_A <- (apply(OTU_table[, grepl("^Af[0-9]+$", colnames(OTU_table))],1,sum))
...
OTU_table$Labeled_B <- (apply(OTU_table[, grepl("^Bf[0-9]+$", colnames(OTU_table))],1,sum))
OTU_table$Labeled_D <- (apply(OTU_table[, grepl("^Df[0-9]+$", colnames(OTU_table))],1,sum))
OTU_table$Labeled_F <- (apply(OTU_table[, grepl("^Ff[0-9]+$", colnames(OTU_table))],1,sum))

OTU_table <- OTU_table %>%
  filter(rowSums(select(., Unlabeled_A, Unlabeled_C, Unlabeled_E) >= 5) >= 2,
         rowSums(select(., Labeled_B, Labeled_D, Labeled_F) >= 5) >= 2)

OTU_table_samples <- select(OTU_table, matches("OTU|^[A-F]f[0-9]+$"))
...
### AWI2 taxa information

OTU_table_taxa <- select(OTU_table, matches("OTU|Domain|Phylum|Subphylum|Class|Subclass|Order|Family|Genus|Species"))
OTU_table_taxa <- OTU_table_taxa %>%
  select(-Subphylum, -Subclass)
